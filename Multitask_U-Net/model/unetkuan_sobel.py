...
def unet(num_classes, input_shape, lr_init, lr_decay, vgg_weight_path=None):
    ...
    img_input_sobel = Lambda(lambda x: modified_sobel(x))(img_input)
    unet_input = multiply([img_input, img_input_sobel])
    # Block 1
    ...
    x_sobel = Lambda(lambda x: modified_sobel(x))(x)
    x = multiply([x, x_sobel])
