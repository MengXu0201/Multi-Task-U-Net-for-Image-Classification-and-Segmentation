...
from model.classificationNet import create_cls_model
from dataset_parser.generator import data_generator_pair, data_generator_cls
from keras.callbacks import ModelCheckpoint

def train_cls_model_handler():

    model_name = 'single_cls_net'
    cls_loss_weight = 0.5
    print('cls loss:{}'.format(cls_loss_weight))
    loss_str = 'loss_cls_{}'.format(str(cls_loss_weight).replace('.', '_'))
...
##data2
    ...
    train_gen = data_generator_cls(nameList=train_name, cLabel=clabel_train,
                                    image_shape=(cls_img_height, cls_img_width, 1), nb_class=2,
                                    b_size=batch_size, mode='train')
    test_gen = data_generator_cls(nameList=val_name, cLabel=clabel_val,
                                    image_shape=(cls_img_height, cls_img_width, 1), nb_class=2,
    ...
    cls_model = create_cls_model( cls_width=cls_img_width,
    ...
    cls_model.fit_generator(train_gen,
